# Claude Design Prompt - Aya Vision 8B
...
- Architecture / modality note: Multilingual multimodal VLM; Command-R-7B plus SigLIP.
- Source strength: Deep table
- Source lead: https://huggingface.co/models?search=Aya+Vision+8B
...
```text
You are Claude Design. Create a standalone, downloadable HTML visualization page for Aya Vision 8B.

Primary source leads to use:
- Hugging Face lookup for Aya Vision 8B: https://huggingface.co/models?search=Aya+Vision+8B
- Local inventory provenance: /Users/rigarg/Desktop/agents/model-atlas/oss-model-releases-2025-plus.md
...
- "Aya Vision 8B" "model card"
- "Aya Vision 8B" "technical report"
- "Aya Vision 8B" "architecture" "config"
- "Aya Vision 8B" "benchmark" "Cohere Labs"

Primary-source checklist for this model:
- Official model card or repository from Cohere Labs.
- Official technical report, paper, release blog, or documentation page for Aya Vision 8B.
...
- Target page title: "Aya Vision 8B Architecture"
...
Goal:
Build a visual architecture atlas page that explains Aya Vision 8B from input tokens or modality inputs to output behavior. The page should be accurate to verified sources, visually rich, and specific to Aya Vision 8B. Do not make a generic transformer explainer.
...
2. Block internals:
   - Draw the block structure implied by verified sources and the inventory note: Multilingual multimodal VLM; Command-R-7B plus SigLIP.
...
- The first viewport should immediately identify Aya Vision 8B and show the high-level specs from the inventory.
...
- Mirror the verified report or model-card structure for Aya Vision 8B when available.
- Avoid generic statements like "uses transformer blocks" unless paired with the concrete mechanism and dimensions verified for Aya Vision 8B.
```
