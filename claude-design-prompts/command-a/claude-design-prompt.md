# Claude Design Prompt - Command A
...
- Lab: Cohere
- Size / active size: 111B
- License / openness: CC-BY-NC
- Architecture / modality note: Autoregressive enterprise text model; sliding-window/global attention; 256K context.
- Source strength: Deep table
- Source lead: https://huggingface.co/models?search=Command+A+111B
...
```text
You are Claude Design. Create a standalone, downloadable HTML visualization page for Command A.

Primary source leads to use:
- Hugging Face lookup for Command A: https://huggingface.co/models?search=Command+A+111B
- Local inventory provenance: /Users/rigarg/Desktop/agents/model-atlas/oss-model-releases-2025-plus.md
...
- "Command A" "architecture" "config"
- "Command A" "benchmark" "Cohere"

Primary-source checklist for this model:
- Official model card or repository from Cohere.
- Official technical report, paper, release blog, or documentation page for Command A.
...
- Release year: 2025
- Parameter scale: 111B
- Active parameter scale: unknown in source inventory
- License or openness: CC-BY-NC
- Inventory source strength: Deep table
- Target page title: "Command A Architecture"
...
Goal:
Build a visual architecture atlas page that explains Command A from input tokens or modality inputs to output behavior. The page should be accurate to verified sources, visually rich, and specific to Command A. Do not make a generic transformer explainer.
...
2. Block internals:
   - Draw the block structure implied by verified sources and the inventory note: Autoregressive enterprise text model; sliding-window/global attention; 256K context.
...
5. Model-specific sections to create from the inventory leads:
   - Autoregressive enterprise text model
...
- The first viewport should immediately identify Command A and show the high-level specs from the inventory.
...
- Mirror the verified report or model-card structure for Command A when available.
- Avoid generic statements like "uses transformer blocks" unless paired with the concrete mechanism and dimensions verified for Command A.
```
