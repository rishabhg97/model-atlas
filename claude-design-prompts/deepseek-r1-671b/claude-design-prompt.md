# Claude Design Prompt - DeepSeek R1 671B
...
- Architecture / modality note: Sparse MoE Transformer; MLA; MTP; reasoning model; R1-Zero/distillation also mentioned.
- Source strength: Both tables
- Source lead: https://huggingface.co/models?search=DeepSeek+R1+671B
...
```text
You are Claude Design. Create a standalone, downloadable HTML visualization page for DeepSeek R1 671B.

Primary source leads to use:
- Hugging Face lookup for DeepSeek R1 671B: https://huggingface.co/models?search=DeepSeek+R1+671B
- Local inventory provenance: /Users/rigarg/Desktop/agents/model-atlas/oss-model-releases-2025-plus.md
...
- "DeepSeek R1 671B" "model card"
- "DeepSeek R1 671B" "technical report"
- "DeepSeek R1 671B" "architecture" "config"
- "DeepSeek R1 671B" "benchmark" "DeepSeek"

Primary-source checklist for this model:
- Official model card or repository from DeepSeek.
- Official technical report, paper, release blog, or documentation page for DeepSeek R1 671B.
...
- Release/report date: 2025-01-20
- Release year: 2025
...
- Target page title: "DeepSeek R1 671B Architecture"
...
Goal:
Build a visual architecture atlas page that explains DeepSeek R1 671B from input tokens or modality inputs to output behavior. The page should be accurate to verified sources, visually rich, and specific to DeepSeek R1 671B. Do not make a generic transformer explainer.
...
2. Block internals:
   - Draw the block structure implied by verified sources and the inventory note: Sparse MoE Transformer; MLA; MTP; reasoning model; R1-Zero/distillation also mentioned.
...
5. Model-specific sections to create from the inventory leads:
   ...
   - MTP
   - reasoning model
   - R1-Zero/distillation also mentioned
...
- The first viewport should immediately identify DeepSeek R1 671B and show the high-level specs from the inventory.
...
- Mirror the verified report or model-card structure for DeepSeek R1 671B when available.
- Avoid generic statements like "uses transformer blocks" unless paired with the concrete mechanism and dimensions verified for DeepSeek R1 671B.
```
