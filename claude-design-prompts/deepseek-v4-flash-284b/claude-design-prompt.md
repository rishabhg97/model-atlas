# Claude Design Prompt - DeepSeek V4-Flash 284B
...
- Lab: DeepSeek
- Size / active size: 284B
- License / openness: MIT
- Architecture / modality note: Sparse MoE Transformer; CSA/HCA and multi-head compression.
- Source strength: Raschka table
- Source lead: https://huggingface.co/models?search=DeepSeek+V4-Flash+284B
...
```text
You are Claude Design. Create a standalone, downloadable HTML visualization page for DeepSeek V4-Flash 284B.

Primary source leads to use:
- Hugging Face lookup for DeepSeek V4-Flash 284B: https://huggingface.co/models?search=DeepSeek+V4-Flash+284B
- Local inventory provenance: /Users/rigarg/Desktop/agents/model-atlas/oss-model-releases-2025-plus.md
...
- "DeepSeek V4-Flash 284B" "model card"
- "DeepSeek V4-Flash 284B" "technical report"
- "DeepSeek V4-Flash 284B" "architecture" "config"
- "DeepSeek V4-Flash 284B" "benchmark" "DeepSeek"

Primary-source checklist for this model:
- Official model card or repository from DeepSeek.
- Official technical report, paper, release blog, or documentation page for DeepSeek V4-Flash 284B.
...
Goal:
Build a visual architecture atlas page that explains DeepSeek V4-Flash 284B from input tokens or modality inputs to output behavior. The page should be accurate to verified sources, visually rich, and specific to DeepSeek V4-Flash 284B. Do not make a generic transformer explainer.
...
2. Block internals:
   - Draw the block structure implied by verified sources and the inventory note: Sparse MoE Transformer; CSA/HCA and multi-head compression.
...
3. Attention and sequence handling:
   - Explain the verified attention variant. Inventory lead: CSA/HCA.
...
5. Model-specific sections to create from the inventory leads:
   ...
   - CSA/HCA and multi-head compression
...
- The first viewport should immediately identify DeepSeek V4-Flash 284B and show the high-level specs from the inventory.
...
- Mirror the verified report or model-card structure for DeepSeek V4-Flash 284B when available.
- Avoid generic statements like "uses transformer blocks" unless paired with the concrete mechanism and dimensions verified for DeepSeek V4-Flash 284B.
```
