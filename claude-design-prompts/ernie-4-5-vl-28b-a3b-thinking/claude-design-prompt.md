# Claude Design Prompt - ERNIE-4.5-VL-28B-A3B-Thinking
...
- Type / modality: Vision-language reasoning MoE
- Size / active size: 28B total / 3B active
- License / openness: Apache-2.0 per backlog row
- Architecture / modality note: 28B-total / 3B-active VLM for visual reasoning, grounding, tool use, and video understanding.
- Local inventory provenance: /Users/rigarg/Desktop/agents/model-atlas/oss-model-releases-2025-plus.md

```text
You are Claude Design. Create a standalone, downloadable HTML visualization page for ERNIE-4.5-VL-28B-A3B-Thinking.

Primary sources to use:
- ERNIE-4.5-VL-28B-A3B-Thinking model card: https://huggingface.co/baidu/ERNIE-4.5-VL-28B-A3B-Thinking
...
- Release/report date: 2025-11
- Type/modality: Vision-language reasoning MoE
- Parameter scale: 28B total / 3B active
- License/openness: Apache-2.0 per backlog row
- Target page title: "ERNIE-4.5-VL-28B-A3B-Thinking Architecture"

Variants or scope to cover:
- ERNIE-4.5-VL-28B-A3B-Thinking

Known source-backed facts from the backlog row:
- 28B-total / 3B-active VLM for visual reasoning, grounding, tool use, and video understanding.
...
- Map image/video input, visual encoder, multimodal projector/fusion, MoE language reasoning blocks, grounding/tool-use outputs, and text generation.
- Show the thinking path separately from final answer decoding only if the model card documents it.
- Include video understanding and visual grounding panels with source-backed input/output limits.
...
- Vision encoder, frame sampling, MoE expert count/top-k, context length, and benchmark numbers are not reported in the backlog row.
...
- Avoid generic statements unless paired with the concrete mechanism, modality path, and dimensions used by ERNIE-4.5-VL-28B-A3B-Thinking.
```
