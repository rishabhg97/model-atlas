# Claude Design Prompt - Falcon-H1 34B
...
- Size / active size: 34B
- License / openness: Falcon License
- Architecture / modality note: Largest size explicitly mentioned in Falcon-H1 range; up to 262K context.
- Source strength: Deep table
- Source lead: https://huggingface.co/models?search=Falcon-H1+34B
...
```text
You are Claude Design. Create a standalone, downloadable HTML visualization page for Falcon-H1 34B.

Primary source leads to use:
- Hugging Face lookup for Falcon-H1 34B: https://huggingface.co/models?search=Falcon-H1+34B
- Local inventory provenance: /Users/rigarg/Desktop/agents/model-atlas/oss-model-releases-2025-plus.md
...
- "Falcon-H1 34B" "model card"
- "Falcon-H1 34B" "technical report"
- "Falcon-H1 34B" "architecture" "config"
- "Falcon-H1 34B" "benchmark" "Technology Innovation Institute"

Primary-source checklist for this model:
- Official model card or repository from Technology Innovation Institute.
- Official technical report, paper, release blog, or documentation page for Falcon-H1 34B.
...
- Target page title: "Falcon-H1 34B Architecture"
...
Goal:
Build a visual architecture atlas page that explains Falcon-H1 34B from input tokens or modality inputs to output behavior. The page should be accurate to verified sources, visually rich, and specific to Falcon-H1 34B. Do not make a generic transformer explainer.
...
2. Block internals:
   - Draw the block structure implied by verified sources and the inventory note: Largest size explicitly mentioned in Falcon-H1 range; up to 262K context.
...
5. Model-specific sections to create from the inventory leads:
   - Largest size explicitly mentioned in Falcon-H1 range
   - up to 262K context
...
- The first viewport should immediately identify Falcon-H1 34B and show the high-level specs from the inventory.
...
- Mirror the verified report or model-card structure for Falcon-H1 34B when available.
- Avoid generic statements like "uses transformer blocks" unless paired with the concrete mechanism and dimensions verified for Falcon-H1 34B.
```
