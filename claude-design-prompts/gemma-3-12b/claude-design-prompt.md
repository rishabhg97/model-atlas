# Claude Design Prompt - Gemma 3 12B
...
- Architecture / modality note: Dense text+vision family; GQA, local/global attention.
- Source strength: Deep table
- Source lead: https://huggingface.co/models?search=Gemma+3+12B
...
```text
You are Claude Design. Create a standalone, downloadable HTML visualization page for Gemma 3 12B.

Primary source leads to use:
- Hugging Face lookup for Gemma 3 12B: https://huggingface.co/models?search=Gemma+3+12B
- Local inventory provenance: /Users/rigarg/Desktop/agents/model-atlas/oss-model-releases-2025-plus.md
...
- "Gemma 3 12B" "model card"
- "Gemma 3 12B" "technical report"
- "Gemma 3 12B" "architecture" "config"
- "Gemma 3 12B" "benchmark" "Google DeepMind"

Primary-source checklist for this model:
- Official model card or repository from Google DeepMind.
- Official technical report, paper, release blog, or documentation page for Gemma 3 12B.
...
- Target page title: "Gemma 3 12B Architecture"
...
Goal:
Build a visual architecture atlas page that explains Gemma 3 12B from input tokens or modality inputs to output behavior. The page should be accurate to verified sources, visually rich, and specific to Gemma 3 12B. Do not make a generic transformer explainer.
...
2. Block internals:
   - Draw the block structure implied by verified sources and the inventory note: Dense text+vision family; GQA, local/global attention.
...
- The first viewport should immediately identify Gemma 3 12B and show the high-level specs from the inventory.
...
- Mirror the verified report or model-card structure for Gemma 3 12B when available.
- Avoid generic statements like "uses transformer blocks" unless paired with the concrete mechanism and dimensions verified for Gemma 3 12B.
```
