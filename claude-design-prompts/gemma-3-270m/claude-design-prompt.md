# Claude Design Prompt - Gemma 3 270M
...
- Lab: Google DeepMind
- Size / active size: 270M
- License / openness: Gemma terms
- Architecture / modality note: Dense Transformer; MQA; QK-Norm; local-global attention.
- Source strength: Raschka table
- Source lead: https://huggingface.co/models?search=Gemma+3+270M
...
```text
You are Claude Design. Create a standalone, downloadable HTML visualization page for Gemma 3 270M.

Primary source leads to use:
- Hugging Face lookup for Gemma 3 270M: https://huggingface.co/models?search=Gemma+3+270M
- Local inventory provenance: /Users/rigarg/Desktop/agents/model-atlas/oss-model-releases-2025-plus.md
...
- "Gemma 3 270M" "technical report"
- "Gemma 3 270M" "architecture" "config"
- "Gemma 3 270M" "benchmark" "Google DeepMind"

Primary-source checklist for this model:
- Official model card or repository from Google DeepMind.
- Official technical report, paper, release blog, or documentation page for Gemma 3 270M.
...
Goal:
Build a visual architecture atlas page that explains Gemma 3 270M from input tokens or modality inputs to output behavior. The page should be accurate to verified sources, visually rich, and specific to Gemma 3 270M. Do not make a generic transformer explainer.
...
2. Block internals:
   - Draw the block structure implied by verified sources and the inventory note: Dense Transformer; MQA; QK-Norm; local-global attention.
...
- The first viewport should immediately identify Gemma 3 270M and show the high-level specs from the inventory.
...
- Mirror the verified report or model-card structure for Gemma 3 270M when available.
- Avoid generic statements like "uses transformer blocks" unless paired with the concrete mechanism and dimensions verified for Gemma 3 270M.
```
