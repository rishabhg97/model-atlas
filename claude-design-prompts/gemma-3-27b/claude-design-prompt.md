# Claude Design Prompt - Gemma 3 27B
...
- Lab: Google DeepMind
- Size / active size: 27B
- License / openness: Gemma terms
- Architecture / modality note: Dense Transformer; GQA; local-global attention; decoder-only/text+vision family.
- Source strength: Both tables
- Source lead: https://huggingface.co/models?search=Gemma+3+27B
...
```text
You are Claude Design. Create a standalone, downloadable HTML visualization page for Gemma 3 27B.

Primary source leads to use:
- Hugging Face lookup for Gemma 3 27B: https://huggingface.co/models?search=Gemma+3+27B
- Local inventory provenance: /Users/rigarg/Desktop/agents/model-atlas/oss-model-releases-2025-plus.md
...
- "Gemma 3 27B" "technical report"
- "Gemma 3 27B" "architecture" "config"
- "Gemma 3 27B" "benchmark" "Google DeepMind"

Primary-source checklist for this model:
- Official model card or repository from Google DeepMind.
- Official technical report, paper, release blog, or documentation page for Gemma 3 27B.
...
- Release/report date: 2025-03-11
- Release year: 2025
- Parameter scale: 27B
- Active parameter scale: unknown in source inventory
- License or openness: Gemma terms
- Inventory source strength: Both tables
- Target page title: "Gemma 3 27B Architecture"
...
Goal:
Build a visual architecture atlas page that explains Gemma 3 27B from input tokens or modality inputs to output behavior. The page should be accurate to verified sources, visually rich, and specific to Gemma 3 27B. Do not make a generic transformer explainer.
...
2. Block internals:
   - Draw the block structure implied by verified sources and the inventory note: Dense Transformer; GQA; local-global attention; decoder-only/text+vision family.
...
5. Model-specific sections to create from the inventory leads:
   ...
   - decoder-only/text+vision family
...
- The first viewport should immediately identify Gemma 3 27B and show the high-level specs from the inventory.
...
- Mirror the verified report or model-card structure for Gemma 3 27B when available.
- Avoid generic statements like "uses transformer blocks" unless paired with the concrete mechanism and dimensions verified for Gemma 3 27B.
```
