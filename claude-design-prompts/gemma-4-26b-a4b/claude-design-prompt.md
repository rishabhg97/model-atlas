# Claude Design Prompt - Gemma 4 26B-A4B
...
- Lab: Google DeepMind
- Size / active size: 26B total / 4B active
- License / openness: Apache-2.0
- Architecture / modality note: Sparse MoE Transformer; local-global attention; decoder-only.
- Source strength: Raschka table
- Source lead: https://huggingface.co/models?search=Gemma+4+26B-A4B
...
```text
You are Claude Design. Create a standalone, downloadable HTML visualization page for Gemma 4 26B-A4B.

Primary source leads to use:
- Hugging Face lookup for Gemma 4 26B-A4B: https://huggingface.co/models?search=Gemma+4+26B-A4B
- Local inventory provenance: /Users/rigarg/Desktop/agents/model-atlas/oss-model-releases-2025-plus.md
...
- "Gemma 4 26B-A4B" "technical report"
- "Gemma 4 26B-A4B" "architecture" "config"
- "Gemma 4 26B-A4B" "benchmark" "Google DeepMind"

Primary-source checklist for this model:
- Official model card or repository from Google DeepMind.
- Official technical report, paper, release blog, or documentation page for Gemma 4 26B-A4B.
...
- Release/report date: 2026-04-02
- Release year: 2026
- Parameter scale: 26B total
- Active parameter scale: 4B active
- License or openness: Apache-2.0
- Inventory source strength: Raschka table
- Target page title: "Gemma 4 26B-A4B Architecture"

Known architecture and modality facts from the inventory:
- Architecture / modality note: Sparse MoE Transformer; local-global attention; decoder-only.
- Attention mechanism lead: local-global attention
...
Goal:
Build a visual architecture atlas page that explains Gemma 4 26B-A4B from input tokens or modality inputs to output behavior. The page should be accurate to verified sources, visually rich, and specific to Gemma 4 26B-A4B. Do not make a generic transformer explainer.
...
2. Block internals:
   - Draw the block structure implied by verified sources and the inventory note: Sparse MoE Transformer; local-global attention; decoder-only.
...
3. Attention and sequence handling:
   - Explain the verified attention variant. Inventory lead: local-global attention.
...
- The first viewport should immediately identify Gemma 4 26B-A4B and show the high-level specs from the inventory.
...
- Mirror the verified report or model-card structure for Gemma 4 26B-A4B when available.
- Avoid generic statements like "uses transformer blocks" unless paired with the concrete mechanism and dimensions verified for Gemma 4 26B-A4B.
```
