# Claude Design Prompt - Gemma 4 31B
...
- Lab: Google DeepMind
- Size / active size: 31B
- License / openness: Apache-2.0
- Architecture / modality note: Dense Transformer; GQA; QK-Norm; local-global attention.
- Source strength: Raschka table
- Source lead: https://huggingface.co/models?search=Gemma+4+31B
...
```text
You are Claude Design. Create a standalone, downloadable HTML visualization page for Gemma 4 31B.

Primary source leads to use:
- Hugging Face lookup for Gemma 4 31B: https://huggingface.co/models?search=Gemma+4+31B
- Local inventory provenance: /Users/rigarg/Desktop/agents/model-atlas/oss-model-releases-2025-plus.md
...
- "Gemma 4 31B" "technical report"
- "Gemma 4 31B" "architecture" "config"
- "Gemma 4 31B" "benchmark" "Google DeepMind"

Primary-source checklist for this model:
- Official model card or repository from Google DeepMind.
- Official technical report, paper, release blog, or documentation page for Gemma 4 31B.
...
- Target page title: "Gemma 4 31B Architecture"
...
Goal:
Build a visual architecture atlas page that explains Gemma 4 31B from input tokens or modality inputs to output behavior. The page should be accurate to verified sources, visually rich, and specific to Gemma 4 31B. Do not make a generic transformer explainer.
...
2. Block internals:
   - Draw the block structure implied by verified sources and the inventory note: Dense Transformer; GQA; QK-Norm; local-global attention.
...
- The first viewport should immediately identify Gemma 4 31B and show the high-level specs from the inventory.
...
- Mirror the verified report or model-card structure for Gemma 4 31B when available.
- Avoid generic statements like "uses transformer blocks" unless paired with the concrete mechanism and dimensions verified for Gemma 4 31B.
```
