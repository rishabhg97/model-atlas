# Claude Design Prompt - GPT-OSS 120B
...
- Lab: OpenAI
- Size / active size: 120B
- License / openness: Apache-2.0
- Architecture / modality note: Sparse MoE Transformer; alternating sliding-window/global attention.
- Source strength: Raschka table
- Source lead: https://huggingface.co/models?search=GPT-OSS+120B
...
```text
You are Claude Design. Create a standalone, downloadable HTML visualization page for GPT-OSS 120B.

Primary source leads to use:
- Hugging Face lookup for GPT-OSS 120B: https://huggingface.co/models?search=GPT-OSS+120B
- Local inventory provenance: /Users/rigarg/Desktop/agents/model-atlas/oss-model-releases-2025-plus.md
...
- "GPT-OSS 120B" "technical report"
- "GPT-OSS 120B" "architecture" "config"
- "GPT-OSS 120B" "benchmark" "OpenAI"

Primary-source checklist for this model:
- Official model card or repository from OpenAI.
- Official technical report, paper, release blog, or documentation page for GPT-OSS 120B.
...
- Model: GPT-OSS 120B
- Family/vendor or lab: OpenAI
- Release/report date: 2025-08-04
- Release year: 2025
...
- Target page title: "GPT-OSS 120B Architecture"
...
Goal:
Build a visual architecture atlas page that explains GPT-OSS 120B from input tokens or modality inputs to output behavior. The page should be accurate to verified sources, visually rich, and specific to GPT-OSS 120B. Do not make a generic transformer explainer.
...
2. Block internals:
   - Draw the block structure implied by verified sources and the inventory note: Sparse MoE Transformer; alternating sliding-window/global attention.
...
- The first viewport should immediately identify GPT-OSS 120B and show the high-level specs from the inventory.
...
- Mirror the verified report or model-card structure for GPT-OSS 120B when available.
- Avoid generic statements like "uses transformer blocks" unless paired with the concrete mechanism and dimensions verified for GPT-OSS 120B.
```
