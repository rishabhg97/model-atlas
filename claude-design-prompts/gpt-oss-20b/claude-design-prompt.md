# Claude Design Prompt - GPT-OSS 20B
...
- Source lead: https://huggingface.co/models?search=GPT-OSS+20B
...
```text
You are Claude Design. Create a standalone, downloadable HTML visualization page for GPT-OSS 20B.

Primary source leads to use:
- Hugging Face lookup for GPT-OSS 20B: https://huggingface.co/models?search=GPT-OSS+20B
- Local inventory provenance: /Users/rigarg/Desktop/agents/model-atlas/oss-model-releases-2025-plus.md
...
- "GPT-OSS 20B" "technical report"
- "GPT-OSS 20B" "architecture" "config"
- "GPT-OSS 20B" "benchmark" "OpenAI"

Primary-source checklist for this model:
- Official model card or repository from OpenAI.
- Official technical report, paper, release blog, or documentation page for GPT-OSS 20B.
...
- Target page title: "GPT-OSS 20B Architecture"
...
Goal:
Build a visual architecture atlas page that explains GPT-OSS 20B from input tokens or modality inputs to output behavior. The page should be accurate to verified sources, visually rich, and specific to GPT-OSS 20B. Do not make a generic transformer explainer.
...
- The first viewport should immediately identify GPT-OSS 20B and show the high-level specs from the inventory.
...
- Mirror the verified report or model-card structure for GPT-OSS 20B when available.
- Avoid generic statements like "uses transformer blocks" unless paired with the concrete mechanism and dimensions verified for GPT-OSS 20B.
```
