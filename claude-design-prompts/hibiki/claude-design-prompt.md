...
- Lab: Kyutai
- Size / active size: size unspecified in report
- License / openness: public weights/code noted
- Architecture / modality note: Decoder-only simultaneous speech-translation model; text and audio token streams.
- Source strength: Deep table
- Source lead: https://huggingface.co/models?search=Hibiki+Kyutai
...
```text
You are Claude Design. Create a standalone, downloadable HTML visualization page for Hibiki.

Primary source leads to use:
- Hugging Face lookup for Hibiki: https://huggingface.co/models?search=Hibiki+Kyutai
- Local inventory provenance: /Users/rigarg/Desktop/agents/model-atlas/oss-model-releases-2025-plus.md
...
- "Hibiki" "model card"
- "Hibiki" "technical report"
- "Hibiki" "architecture" "config"
- "Hibiki" "benchmark" "Kyutai"

Primary-source checklist for this model:
- Official model card or repository from Kyutai.
- Official technical report, paper, release blog, or documentation page for Hibiki.
...
- Model: Hibiki
- Family/vendor or lab: Kyutai
- Release/report date: 2025-02
- Release year: 2025
- Parameter scale: size unspecified in report
- Active parameter scale: unknown in source inventory
- License or openness: public weights/code noted
- Inventory source strength: Deep table
- Target page title: "Hibiki Architecture"
...
Goal:
Build a visual architecture atlas page that explains Hibiki from input tokens or modality inputs to output behavior. The page should be accurate to verified sources, visually rich, and specific to Hibiki. Do not make a generic transformer explainer.
...
2. Block internals:
   - Draw the block structure implied by verified sources and the inventory note: Decoder-only simultaneous speech-translation model; text and audio token streams.
...
5. Model-specific sections to create from the inventory leads:
   - Decoder-only simultaneous speech-translation model
   - text and audio token streams
...
- The first viewport should immediately identify Hibiki and show the high-level specs from the inventory.
...
- Mirror the verified report or model-card structure for Hibiki when available.
- Avoid generic statements like "uses transformer blocks" unless paired with the concrete mechanism and dimensions verified for Hibiki.
```
