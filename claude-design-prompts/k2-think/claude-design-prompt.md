...
- Architecture / modality note: Reasoning system built on Qwen2.5-32B with long-CoT SFT and verifiable-reward RL.
- Source strength: Deep table
- Source lead: https://huggingface.co/models?search=K2-Think
...
```text
You are Claude Design. Create a standalone, downloadable HTML visualization page for K2-Think.

Primary source leads to use:
- Hugging Face lookup for K2-Think: https://huggingface.co/models?search=K2-Think
- Local inventory provenance: /Users/rigarg/Desktop/agents/model-atlas/oss-model-releases-2025-plus.md
...
- "K2-Think" "model card"
- "K2-Think" "technical report"
- "K2-Think" "architecture" "config"
- "K2-Think" "benchmark" "MBZUAI IFM / G42"

Primary-source checklist for this model:
- Official model card or repository from MBZUAI IFM / G42.
- Official technical report, paper, release blog, or documentation page for K2-Think.
...
- Model: K2-Think
- Family/vendor or lab: MBZUAI IFM / G42
...
- Target page title: "K2-Think Architecture"
...
Goal:
Build a visual architecture atlas page that explains K2-Think from input tokens or modality inputs to output behavior. The page should be accurate to verified sources, visually rich, and specific to K2-Think. Do not make a generic transformer explainer.
...
2. Block internals:
   - Draw the block structure implied by verified sources and the inventory note: Reasoning system built on Qwen2.5-32B with long-CoT SFT and verifiable-reward RL.
...
5. Model-specific sections to create from the inventory leads:
   - Reasoning system built on Qwen2.5-32B with long-CoT SFT and verifiable-reward RL
...
- The first viewport should immediately identify K2-Think and show the high-level specs from the inventory.
...
- Mirror the verified report or model-card structure for K2-Think when available.
- Avoid generic statements like "uses transformer blocks" unless paired with the concrete mechanism and dimensions verified for K2-Think.
```
