# Claude Design Prompt - Ling 2.5 1T
...
- Architecture / modality note: Sparse hybrid model; Lightning Attention plus MLA.
- Source strength: Raschka table
- Source lead: https://huggingface.co/models?search=Ling+2.5+1T
...
```text
You are Claude Design. Create a standalone, downloadable HTML visualization page for Ling 2.5 1T.

Primary source leads to use:
- Hugging Face lookup for Ling 2.5 1T: https://huggingface.co/models?search=Ling+2.5+1T
- Local inventory provenance: /Users/rigarg/Desktop/agents/model-atlas/oss-model-releases-2025-plus.md
...
- "Ling 2.5 1T" "model card"
- "Ling 2.5 1T" "technical report"
- "Ling 2.5 1T" "architecture" "config"
- "Ling 2.5 1T" "benchmark" "inclusionAI"

Primary-source checklist for this model:
- Official model card or repository from inclusionAI.
- Official technical report, paper, release blog, or documentation page for Ling 2.5 1T.
...
Goal:
Build a visual architecture atlas page that explains Ling 2.5 1T from input tokens or modality inputs to output behavior. The page should be accurate to verified sources, visually rich, and specific to Ling 2.5 1T. Do not make a generic transformer explainer.
...
2. Block internals:
   - Draw the block structure implied by verified sources and the inventory note: Sparse hybrid model; Lightning Attention plus MLA.
...
- The first viewport should immediately identify Ling 2.5 1T and show the high-level specs from the inventory.
...
- Mirror the verified report or model-card structure for Ling 2.5 1T when available.
- Avoid generic statements like "uses transformer blocks" unless paired with the concrete mechanism and dimensions verified for Ling 2.5 1T.
```
