# Claude Design Prompt - Llama-Nemotron Super 49B
...
- Lab: NVIDIA
- Size / active size: 49B
- License / openness: NVIDIA Open Model License
- Architecture / modality note: Llama-derived reasoning model; throughput-focused.
- Source strength: Deep table
- Source lead: https://huggingface.co/models?search=Llama-Nemotron+Super+49B
...
```text
You are Claude Design. Create a standalone, downloadable HTML visualization page for Llama-Nemotron Super 49B.

Primary source leads to use:
- Hugging Face lookup for Llama-Nemotron Super 49B: https://huggingface.co/models?search=Llama-Nemotron+Super+49B
- Local inventory provenance: /Users/rigarg/Desktop/agents/model-atlas/oss-model-releases-2025-plus.md
...
- "Llama-Nemotron Super 49B" "model card"
- "Llama-Nemotron Super 49B" "technical report"
- "Llama-Nemotron Super 49B" "architecture" "config"
- "Llama-Nemotron Super 49B" "benchmark" "NVIDIA"

Primary-source checklist for this model:
- Official model card or repository from NVIDIA.
- Official technical report, paper, release blog, or documentation page for Llama-Nemotron Super 49B.
...
- Target page title: "Llama-Nemotron Super 49B Architecture"
...
Goal:
Build a visual architecture atlas page that explains Llama-Nemotron Super 49B from input tokens or modality inputs to output behavior. The page should be accurate to verified sources, visually rich, and specific to Llama-Nemotron Super 49B. Do not make a generic transformer explainer.
...
2. Block internals:
   - Draw the block structure implied by verified sources and the inventory note: Llama-derived reasoning model; throughput-focused.
...
- The first viewport should immediately identify Llama-Nemotron Super 49B and show the high-level specs from the inventory.
...
- Mirror the verified report or model-card structure for Llama-Nemotron Super 49B when available.
- Avoid generic statements like "uses transformer blocks" unless paired with the concrete mechanism and dimensions verified for Llama-Nemotron Super 49B.
```
