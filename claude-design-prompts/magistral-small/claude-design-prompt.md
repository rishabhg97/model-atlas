# Claude Design Prompt - Magistral Small
...
- Lab: Mistral AI
- Size / active size: built on Mistral Small 3.1; size not repeated in source row
- License / openness: public HF card noted
- Architecture / modality note: Reasoning model using RLVR framework.
- Source strength: Deep table
- Source lead: https://huggingface.co/models?search=Magistral+Small
...
```text
You are Claude Design. Create a standalone, downloadable HTML visualization page for Magistral Small.

Primary source leads to use:
- Hugging Face lookup for Magistral Small: https://huggingface.co/models?search=Magistral+Small
- Local inventory provenance: /Users/rigarg/Desktop/agents/model-atlas/oss-model-releases-2025-plus.md
...
- "Magistral Small" "model card"
- "Magistral Small" "technical report"
- "Magistral Small" "architecture" "config"
- "Magistral Small" "benchmark" "Mistral AI"

Primary-source checklist for this model:
- Official model card or repository from Mistral AI.
- Official technical report, paper, release blog, or documentation page for Magistral Small.
...
- Model: Magistral Small
- Family/vendor or lab: Mistral AI
- Release/report date: 2025-06
- Release year: 2025
- Parameter scale: built on Mistral Small 3.1; size not repeated in source row
- Active parameter scale: unknown in source inventory
- License or openness: public HF card noted
- Inventory source strength: Deep table
- Target page title: "Magistral Small Architecture"
...
Goal:
Build a visual architecture atlas page that explains Magistral Small from input tokens or modality inputs to output behavior. The page should be accurate to verified sources, visually rich, and specific to Magistral Small. Do not make a generic transformer explainer.
...
2. Block internals:
   - Draw the block structure implied by verified sources and the inventory note: Reasoning model using RLVR framework.
...
5. Model-specific sections to create from the inventory leads:
   - Reasoning model using RLVR framework
...
- The first viewport should immediately identify Magistral Small and show the high-level specs from the inventory.
...
- Mirror the verified report or model-card structure for Magistral Small when available.
- Avoid generic statements like "uses transformer blocks" unless paired with the concrete mechanism and dimensions verified for Magistral Small.
```
