# Claude Design Prompt - MedGemma 1.5 4B

Metadata from source inventory:
- Model: MedGemma 1.5 4B
- Year: 2026
- Release date: 2026-04
- Lab: Google Health / Google DeepMind
- Size / active size: 4B multimodal
- License / openness: public weights noted
- Architecture / modality note: Medical multimodal model extended to CT/MRI, pathology, documents, and EHR.
- Source strength: Deep table
- Source lead: https://huggingface.co/models?search=MedGemma+1.5+4B
...
```text
You are Claude Design. Create a standalone, downloadable HTML visualization page for MedGemma 1.5 4B.

Primary source leads to use:
- Hugging Face lookup for MedGemma 1.5 4B: https://huggingface.co/models?search=MedGemma+1.5+4B
- Local inventory provenance: /Users/rigarg/Desktop/agents/model-atlas/oss-model-releases-2025-plus.md
...
- "MedGemma 1.5 4B" "model card"
- "MedGemma 1.5 4B" "technical report"
- "MedGemma 1.5 4B" "architecture" "config"
- "MedGemma 1.5 4B" "benchmark" "Google Health / Google DeepMind"

Primary-source checklist for this model:
- Official model card or repository from Google Health / Google DeepMind.
- Official technical report, paper, release blog, or documentation page for MedGemma 1.5 4B.
...
- Model: MedGemma 1.5 4B
- Family/vendor or lab: Google Health / Google DeepMind
- Release/report date: 2026-04
- Release year: 2026
- Parameter scale: 4B multimodal
- Active parameter scale: unknown in source inventory
- License or openness: public weights noted
- Inventory source strength: Deep table
- Target page title: "MedGemma 1.5 4B Architecture"
...
Goal:
Build a visual architecture atlas page that explains MedGemma 1.5 4B from input tokens or modality inputs to output behavior. The page should be accurate to verified sources, visually rich, and specific to MedGemma 1.5 4B. Do not make a generic transformer explainer.
...
2. Block internals:
   - Draw the block structure implied by verified sources and the inventory note: Medical multimodal model extended to CT/MRI, pathology, documents, and EHR.
...
5. Model-specific sections to create from the inventory leads:
   - Medical multimodal model extended to CT/MRI
   - pathology
   - documents
   - and EHR
...
- The first viewport should immediately identify MedGemma 1.5 4B and show the high-level specs from the inventory.
...
- Mirror the verified report or model-card structure for MedGemma 1.5 4B when available.
- Avoid generic statements like "uses transformer blocks" unless paired with the concrete mechanism and dimensions verified for MedGemma 1.5 4B.
```
