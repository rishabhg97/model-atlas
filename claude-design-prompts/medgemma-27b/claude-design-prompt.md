# Claude Design Prompt - MedGemma 27B
...
- Size / active size: 27B text
- License / openness: public weights noted
- Architecture / modality note: Medical text model for clinical/biomedical tasks.
- Source strength: Deep table
- Source lead: https://huggingface.co/models?search=MedGemma+27B
...
```text
You are Claude Design. Create a standalone, downloadable HTML visualization page for MedGemma 27B.

Primary source leads to use:
- Hugging Face lookup for MedGemma 27B: https://huggingface.co/models?search=MedGemma+27B
- Local inventory provenance: /Users/rigarg/Desktop/agents/model-atlas/oss-model-releases-2025-plus.md
...
- "MedGemma 27B" "model card"
- "MedGemma 27B" "technical report"
- "MedGemma 27B" "architecture" "config"
- "MedGemma 27B" "benchmark" "Google Health / Google DeepMind"

Primary-source checklist for this model:
- Official model card or repository from Google Health / Google DeepMind.
- Official technical report, paper, release blog, or documentation page for MedGemma 27B.
...
- Target page title: "MedGemma 27B Architecture"
...
Goal:
Build a visual architecture atlas page that explains MedGemma 27B from input tokens or modality inputs to output behavior. The page should be accurate to verified sources, visually rich, and specific to MedGemma 27B. Do not make a generic transformer explainer.
...
2. Block internals:
   - Draw the block structure implied by verified sources and the inventory note: Medical text model for clinical/biomedical tasks.
...
5. Model-specific sections to create from the inventory leads:
   - Medical text model for clinical/biomedical tasks
...
- The first viewport should immediately identify MedGemma 27B and show the high-level specs from the inventory.
...
- Mirror the verified report or model-card structure for MedGemma 27B when available.
- Avoid generic statements like "uses transformer blocks" unless paired with the concrete mechanism and dimensions verified for MedGemma 27B.
```
