# Claude Design Prompt - MedGemma 4B
...
- Architecture / modality note: Medical Gemma 3 derivative with MedSigLIP image encoder.
- Source strength: Deep table
- Source lead: https://huggingface.co/models?search=MedGemma+4B
...
```text
You are Claude Design. Create a standalone, downloadable HTML visualization page for MedGemma 4B.

Primary source leads to use:
- Hugging Face lookup for MedGemma 4B: https://huggingface.co/models?search=MedGemma+4B
- Local inventory provenance: /Users/rigarg/Desktop/agents/model-atlas/oss-model-releases-2025-plus.md
...
- "MedGemma 4B" "model card"
- "MedGemma 4B" "technical report"
- "MedGemma 4B" "architecture" "config"
- "MedGemma 4B" "benchmark" "Google Health / Google DeepMind"

Primary-source checklist for this model:
- Official model card or repository from Google Health / Google DeepMind.
- Official technical report, paper, release blog, or documentation page for MedGemma 4B.
...
- Target page title: "MedGemma 4B Architecture"
...
Goal:
Build a visual architecture atlas page that explains MedGemma 4B from input tokens or modality inputs to output behavior. The page should be accurate to verified sources, visually rich, and specific to MedGemma 4B. Do not make a generic transformer explainer.
...
2. Block internals:
   - Draw the block structure implied by verified sources and the inventory note: Medical Gemma 3 derivative with MedSigLIP image encoder.
...
5. Model-specific sections to create from the inventory leads:
   - Medical Gemma 3 derivative with MedSigLIP image encoder
...
- The first viewport should immediately identify MedGemma 4B and show the high-level specs from the inventory.
...
- Mirror the verified report or model-card structure for MedGemma 4B when available.
- Avoid generic statements like "uses transformer blocks" unless paired with the concrete mechanism and dimensions verified for MedGemma 4B.
```
