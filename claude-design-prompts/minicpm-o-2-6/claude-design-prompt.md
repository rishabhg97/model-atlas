# Claude Design Prompt - MiniCPM-o 2.6
...
- Architecture / modality note: End-to-end omni-modal model for image, video, text, audio input and text/speech output.
- Source strength: Deep table
- Source lead: https://huggingface.co/models?search=MiniCPM-o+2.6
...
```text
You are Claude Design. Create a standalone, downloadable HTML visualization page for MiniCPM-o 2.6.

Primary source leads to use:
- Hugging Face lookup for MiniCPM-o 2.6: https://huggingface.co/models?search=MiniCPM-o+2.6
- Local inventory provenance: /Users/rigarg/Desktop/agents/model-atlas/oss-model-releases-2025-plus.md
...
- "MiniCPM-o 2.6" "technical report"
- "MiniCPM-o 2.6" "architecture" "config"
- "MiniCPM-o 2.6" "benchmark" "OpenBMB"

Primary-source checklist for this model:
- Official model card or repository from OpenBMB.
- Official technical report, paper, release blog, or documentation page for MiniCPM-o 2.6.
...
- Target page title: "MiniCPM-o 2.6 Architecture"
...
Goal:
Build a visual architecture atlas page that explains MiniCPM-o 2.6 from input tokens or modality inputs to output behavior. The page should be accurate to verified sources, visually rich, and specific to MiniCPM-o 2.6. Do not make a generic transformer explainer.
...
2. Block internals:
   - Draw the block structure implied by verified sources and the inventory note: End-to-end omni-modal model for image, video, text, audio input and text/speech output.
...
5. Model-specific sections to create from the inventory leads:
   - End-to-end omni-modal model for image
   - video
   - text
   - audio input and text/speech output
...
- The first viewport should immediately identify MiniCPM-o 2.6 and show the high-level specs from the inventory.
...
- Mirror the verified report or model-card structure for MiniCPM-o 2.6 when available.
- Avoid generic statements like "uses transformer blocks" unless paired with the concrete mechanism and dimensions verified for MiniCPM-o 2.6.
```
