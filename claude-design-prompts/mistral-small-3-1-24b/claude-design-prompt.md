# Claude Design Prompt - Mistral Small 3.1 24B
...
- Lab: Mistral AI
- Size / active size: 24B
- License / openness: Apache-2.0
- Architecture / modality note: Dense Transformer; GQA; RoPE; decoder-only.
- Source strength: Raschka table
- Source lead: https://huggingface.co/models?search=Mistral+Small+3.1+24B
...
```text
You are Claude Design. Create a standalone, downloadable HTML visualization page for Mistral Small 3.1 24B.

Primary source leads to use:
- Hugging Face lookup for Mistral Small 3.1 24B: https://huggingface.co/models?search=Mistral+Small+3.1+24B
- Local inventory provenance: /Users/rigarg/Desktop/agents/model-atlas/oss-model-releases-2025-plus.md
...
- "Mistral Small 3.1 24B" "model card"
- "Mistral Small 3.1 24B" "technical report"
- "Mistral Small 3.1 24B" "architecture" "config"
- "Mistral Small 3.1 24B" "benchmark" "Mistral AI"

Primary-source checklist for this model:
- Official model card or repository from Mistral AI.
- Official technical report, paper, release blog, or documentation page for Mistral Small 3.1 24B.
...
Goal:
Build a visual architecture atlas page that explains Mistral Small 3.1 24B from input tokens or modality inputs to output behavior. The page should be accurate to verified sources, visually rich, and specific to Mistral Small 3.1 24B. Do not make a generic transformer explainer.
...
2. Block internals:
   - Draw the block structure implied by verified sources and the inventory note: Dense Transformer; GQA; RoPE; decoder-only.
...
- The first viewport should immediately identify Mistral Small 3.1 24B and show the high-level specs from the inventory.
...
- Mirror the verified report or model-card structure for Mistral Small 3.1 24B when available.
- Avoid generic statements like "uses transformer blocks" unless paired with the concrete mechanism and dimensions verified for Mistral Small 3.1 24B.
```
