# Claude Design Prompt - MobiLlama
...
- Release date: 2025
- Lab: MBZUAI
- Size / active size: 1.2B large-base highlighted
- License / openness: open repo/HF card noted
- Architecture / modality note: Small/mobile language model family; 1.2T-token pretraining for highlighted variant.
- Source strength: Deep table
- Source lead: https://huggingface.co/models?search=MobiLlama
...
```text
You are Claude Design. Create a standalone, downloadable HTML visualization page for MobiLlama.

Primary source leads to use:
- Hugging Face lookup for MobiLlama: https://huggingface.co/models?search=MobiLlama
- Local inventory provenance: /Users/rigarg/Desktop/agents/model-atlas/oss-model-releases-2025-plus.md
...
- "MobiLlama" "model card"
- "MobiLlama" "technical report"
- "MobiLlama" "architecture" "config"
- "MobiLlama" "benchmark" "MBZUAI"

Primary-source checklist for this model:
- Official model card or repository from MBZUAI.
- Official technical report, paper, release blog, or documentation page for MobiLlama.
...
- Model: MobiLlama
- Family/vendor or lab: MBZUAI
- Release/report date: 2025
- Release year: 2025
- Parameter scale: 1.2B large-base highlighted
- Active parameter scale: unknown in source inventory
- License or openness: open repo/HF card noted
- Inventory source strength: Deep table
- Target page title: "MobiLlama Architecture"
...
Goal:
Build a visual architecture atlas page that explains MobiLlama from input tokens or modality inputs to output behavior. The page should be accurate to verified sources, visually rich, and specific to MobiLlama. Do not make a generic transformer explainer.
...
2. Block internals:
   - Draw the block structure implied by verified sources and the inventory note: Small/mobile language model family; 1.2T-token pretraining for highlighted variant.
...
5. Model-specific sections to create from the inventory leads:
   - Small/mobile language model family
   - 1.2T-token pretraining for highlighted variant
...
- The first viewport should immediately identify MobiLlama and show the high-level specs from the inventory.
...
- Mirror the verified report or model-card structure for MobiLlama when available.
- Avoid generic statements like "uses transformer blocks" unless paired with the concrete mechanism and dimensions verified for MobiLlama.
```
