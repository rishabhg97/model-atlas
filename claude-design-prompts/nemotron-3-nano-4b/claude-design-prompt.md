# Claude Design Prompt - Nemotron 3 Nano 4B
...
- Architecture / modality note: Dense hybrid model; Mamba-2 layers plus four GQA layers.
- Source strength: Raschka table
- Source lead: https://huggingface.co/models?search=Nemotron+3+Nano+4B
...
```text
You are Claude Design. Create a standalone, downloadable HTML visualization page for Nemotron 3 Nano 4B.

Primary source leads to use:
- Hugging Face lookup for Nemotron 3 Nano 4B: https://huggingface.co/models?search=Nemotron+3+Nano+4B
- Local inventory provenance: /Users/rigarg/Desktop/agents/model-atlas/oss-model-releases-2025-plus.md
...
- "Nemotron 3 Nano 4B" "model card"
- "Nemotron 3 Nano 4B" "technical report"
- "Nemotron 3 Nano 4B" "architecture" "config"
- "Nemotron 3 Nano 4B" "benchmark" "NVIDIA"

Primary-source checklist for this model:
- Official model card or repository from NVIDIA.
- Official technical report, paper, release blog, or documentation page for Nemotron 3 Nano 4B.
...
- Release/report date: 2026-03-16
- Release year: 2026
- Parameter scale: 4B
...
- Target page title: "Nemotron 3 Nano 4B Architecture"
...
Goal:
Build a visual architecture atlas page that explains Nemotron 3 Nano 4B from input tokens or modality inputs to output behavior. The page should be accurate to verified sources, visually rich, and specific to Nemotron 3 Nano 4B. Do not make a generic transformer explainer.
...
2. Block internals:
   - Draw the block structure implied by verified sources and the inventory note: Dense hybrid model; Mamba-2 layers plus four GQA layers.
...
5. Model-specific sections to create from the inventory leads:
   - Dense hybrid model
   - Mamba-2 layers plus four GQA layers
...
- The first viewport should immediately identify Nemotron 3 Nano 4B and show the high-level specs from the inventory.
...
- Mirror the verified report or model-card structure for Nemotron 3 Nano 4B when available.
- Avoid generic statements like "uses transformer blocks" unless paired with the concrete mechanism and dimensions verified for Nemotron 3 Nano 4B.
```
