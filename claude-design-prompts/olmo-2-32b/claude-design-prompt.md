# Claude Design Prompt - OLMo 2 32B
...
- Size / active size: 32B
- License / openness: Apache-2.0
- Architecture / modality note: Fully open dense autoregressive LM; Tulu/RLVR post-training mentioned.
- Source strength: Deep table
- Source lead: https://huggingface.co/models?search=OLMo+2+32B
...
```text
You are Claude Design. Create a standalone, downloadable HTML visualization page for OLMo 2 32B.

Primary source leads to use:
- Hugging Face lookup for OLMo 2 32B: https://huggingface.co/models?search=OLMo+2+32B
- Local inventory provenance: /Users/rigarg/Desktop/agents/model-atlas/oss-model-releases-2025-plus.md
...
- "OLMo 2 32B" "model card"
- "OLMo 2 32B" "technical report"
- "OLMo 2 32B" "architecture" "config"
- "OLMo 2 32B" "benchmark" "Allen Institute for AI"

Primary-source checklist for this model:
- Official model card or repository from Allen Institute for AI.
- Official technical report, paper, release blog, or documentation page for OLMo 2 32B.
...
- Target page title: "OLMo 2 32B Architecture"
...
Goal:
Build a visual architecture atlas page that explains OLMo 2 32B from input tokens or modality inputs to output behavior. The page should be accurate to verified sources, visually rich, and specific to OLMo 2 32B. Do not make a generic transformer explainer.
...
2. Block internals:
   - Draw the block structure implied by verified sources and the inventory note: Fully open dense autoregressive LM; Tulu/RLVR post-training mentioned.
...
5. Model-specific sections to create from the inventory leads:
   - Fully open dense autoregressive LM
   - Tulu/RLVR post-training mentioned
...
- The first viewport should immediately identify OLMo 2 32B and show the high-level specs from the inventory.
...
- Mirror the verified report or model-card structure for OLMo 2 32B when available.
- Avoid generic statements like "uses transformer blocks" unless paired with the concrete mechanism and dimensions verified for OLMo 2 32B.
```
