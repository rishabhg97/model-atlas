# Claude Design Prompt - OLMo 3 32B
...
- Architecture / modality note: Dense Transformer; GQA plus sliding-window attention; decoder-only.
- Source strength: Raschka table
- Source lead: https://huggingface.co/models?search=OLMo+3+32B
...
```text
You are Claude Design. Create a standalone, downloadable HTML visualization page for OLMo 3 32B.

Primary source leads to use:
- Hugging Face lookup for OLMo 3 32B: https://huggingface.co/models?search=OLMo+3+32B
- Local inventory provenance: /Users/rigarg/Desktop/agents/model-atlas/oss-model-releases-2025-plus.md
...
- "OLMo 3 32B" "model card"
- "OLMo 3 32B" "technical report"
- "OLMo 3 32B" "architecture" "config"
- "OLMo 3 32B" "benchmark" "Allen Institute for AI"

Primary-source checklist for this model:
- Official model card or repository from Allen Institute for AI.
- Official technical report, paper, release blog, or documentation page for OLMo 3 32B.
...
- Target page title: "OLMo 3 32B Architecture"
...
Goal:
Build a visual architecture atlas page that explains OLMo 3 32B from input tokens or modality inputs to output behavior. The page should be accurate to verified sources, visually rich, and specific to OLMo 3 32B. Do not make a generic transformer explainer.
...
2. Block internals:
   - Draw the block structure implied by verified sources and the inventory note: Dense Transformer; GQA plus sliding-window attention; decoder-only.
...
5. Model-specific sections to create from the inventory leads:
   - Dense Transformer
   - GQA plus sliding-window attention
   - decoder-only
...
- The first viewport should immediately identify OLMo 3 32B and show the high-level specs from the inventory.
...
- Mirror the verified report or model-card structure for OLMo 3 32B when available.
- Avoid generic statements like "uses transformer blocks" unless paired with the concrete mechanism and dimensions verified for OLMo 3 32B.
```
