# Claude Design Prompt - OLMo 3 7B
...
- Architecture / modality note: Dense Transformer; MHA plus sliding-window attention; decoder-only.
- Source strength: Raschka table
- Source lead: https://huggingface.co/models?search=OLMo+3+7B
...
```text
You are Claude Design. Create a standalone, downloadable HTML visualization page for OLMo 3 7B.

Primary source leads to use:
- Hugging Face lookup for OLMo 3 7B: https://huggingface.co/models?search=OLMo+3+7B
- Local inventory provenance: /Users/rigarg/Desktop/agents/model-atlas/oss-model-releases-2025-plus.md
...
- "OLMo 3 7B" "model card"
- "OLMo 3 7B" "technical report"
- "OLMo 3 7B" "architecture" "config"
- "OLMo 3 7B" "benchmark" "Allen Institute for AI"

Primary-source checklist for this model:
- Official model card or repository from Allen Institute for AI.
- Official technical report, paper, release blog, or documentation page for OLMo 3 7B.
...
Goal:
Build a visual architecture atlas page that explains OLMo 3 7B from input tokens or modality inputs to output behavior. The page should be accurate to verified sources, visually rich, and specific to OLMo 3 7B. Do not make a generic transformer explainer.
...
2. Block internals:
   - Draw the block structure implied by verified sources and the inventory note: Dense Transformer; MHA plus sliding-window attention; decoder-only.
...
3. Attention and sequence handling:
   - Explain the verified attention variant. Inventory lead: MHA.
...
5. Model-specific sections to create from the inventory leads:
   - Dense Transformer
   - MHA plus sliding-window attention
   - decoder-only
...
- The first viewport should immediately identify OLMo 3 7B and show the high-level specs from the inventory.
...
- Mirror the verified report or model-card structure for OLMo 3 7B when available.
- Avoid generic statements like "uses transformer blocks" unless paired with the concrete mechanism and dimensions verified for OLMo 3 7B.
```
