# Claude Design Prompt - Qwen3 30B-A3B
...
- Architecture / modality note: Sparse MoE Transformer; 128 total / 8 active experts in family note.
- Source strength: Both tables
- Source lead: https://huggingface.co/models?search=Qwen3+30B-A3B
...
```text
You are Claude Design. Create a standalone, downloadable HTML visualization page for Qwen3 30B-A3B.

Primary source leads to use:
- Hugging Face lookup for Qwen3 30B-A3B: https://huggingface.co/models?search=Qwen3+30B-A3B
- Local inventory provenance: /Users/rigarg/Desktop/agents/model-atlas/oss-model-releases-2025-plus.md
...
- "Qwen3 30B-A3B" "technical report"
- "Qwen3 30B-A3B" "architecture" "config"
- "Qwen3 30B-A3B" "benchmark" "Alibaba Qwen"

Primary-source checklist for this model:
- Official model card or repository from Alibaba Qwen.
- Official technical report, paper, release blog, or documentation page for Qwen3 30B-A3B.
...
- Target page title: "Qwen3 30B-A3B Architecture"
...
Goal:
Build a visual architecture atlas page that explains Qwen3 30B-A3B from input tokens or modality inputs to output behavior. The page should be accurate to verified sources, visually rich, and specific to Qwen3 30B-A3B. Do not make a generic transformer explainer.
...
2. Block internals:
   - Draw the block structure implied by verified sources and the inventory note: Sparse MoE Transformer; 128 total / 8 active experts in family note.
...
5. Model-specific sections to create from the inventory leads:
   ...
   - 128 total / 8 active experts in family note
...
- The first viewport should immediately identify Qwen3 30B-A3B and show the high-level specs from the inventory.
...
- Mirror the verified report or model-card structure for Qwen3 30B-A3B when available.
- Avoid generic statements like "uses transformer blocks" unless paired with the concrete mechanism and dimensions verified for Qwen3 30B-A3B.
```
