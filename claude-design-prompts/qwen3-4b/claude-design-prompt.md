...
- Architecture / modality note: Dense Transformer; GQA; QK-Norm; decoder-only.
- Source strength: Raschka table
- Source lead: https://huggingface.co/models?search=Qwen3+4B
...
```text
You are Claude Design. Create a standalone, downloadable HTML visualization page for Qwen3 4B.

Primary source leads to use:
- Hugging Face lookup for Qwen3 4B: https://huggingface.co/models?search=Qwen3+4B
- Local inventory provenance: /Users/rigarg/Desktop/agents/model-atlas/oss-model-releases-2025-plus.md
...
- "Qwen3 4B" "model card"
- "Qwen3 4B" "technical report"
- "Qwen3 4B" "architecture" "config"
- "Qwen3 4B" "benchmark" "Alibaba Qwen"

Primary-source checklist for this model:
- Official model card or repository from Alibaba Qwen.
- Official technical report, paper, release blog, or documentation page for Qwen3 4B.
...
- Target page title: "Qwen3 4B Architecture"
...
Goal:
Build a visual architecture atlas page that explains Qwen3 4B from input tokens or modality inputs to output behavior. The page should be accurate to verified sources, visually rich, and specific to Qwen3 4B. Do not make a generic transformer explainer.
...
2. Block internals:
   - Draw the block structure implied by verified sources and the inventory note: Dense Transformer; GQA; QK-Norm; decoder-only.
...
- The first viewport should immediately identify Qwen3 4B and show the high-level specs from the inventory.
...
- Mirror the verified report or model-card structure for Qwen3 4B when available.
- Avoid generic statements like "uses transformer blocks" unless paired with the concrete mechanism and dimensions verified for Qwen3 4B.
```
