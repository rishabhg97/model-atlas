# Claude Design Prompt - Qwen3.6 27B
...
- Architecture / modality note: Dense hybrid model; Gated DeltaNet plus Gated Attention.
- Source strength: Raschka table
- Source lead: https://huggingface.co/models?search=Qwen3.6+27B
...
```text
You are Claude Design. Create a standalone, downloadable HTML visualization page for Qwen3.6 27B.

Primary source leads to use:
- Hugging Face lookup for Qwen3.6 27B: https://huggingface.co/models?search=Qwen3.6+27B
- Local inventory provenance: /Users/rigarg/Desktop/agents/model-atlas/oss-model-releases-2025-plus.md
...
- "Qwen3.6 27B" "technical report"
- "Qwen3.6 27B" "architecture" "config"
- "Qwen3.6 27B" "benchmark" "Alibaba Qwen"

Primary-source checklist for this model:
- Official model card or repository from Alibaba Qwen.
- Official technical report, paper, release blog, or documentation page for Qwen3.6 27B.
...
- Target page title: "Qwen3.6 27B Architecture"
...
Goal:
Build a visual architecture atlas page that explains Qwen3.6 27B from input tokens or modality inputs to output behavior. The page should be accurate to verified sources, visually rich, and specific to Qwen3.6 27B. Do not make a generic transformer explainer.
...
2. Block internals:
   - Draw the block structure implied by verified sources and the inventory note: Dense hybrid model; Gated DeltaNet plus Gated Attention.
...
- The first viewport should immediately identify Qwen3.6 27B and show the high-level specs from the inventory.
...
- Mirror the verified report or model-card structure for Qwen3.6 27B when available.
- Avoid generic statements like "uses transformer blocks" unless paired with the concrete mechanism and dimensions verified for Qwen3.6 27B.
```
