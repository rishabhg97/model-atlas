# Claude Design Prompt - Qwen3 Next 80B-A3B
...
- Lab: Alibaba Qwen
- Size / active size: 80B total / 3B active
...
- Source lead: https://huggingface.co/models?search=Qwen3+Next+80B-A3B
...
```text
You are Claude Design. Create a standalone, downloadable HTML visualization page for Qwen3 Next 80B-A3B.

Primary source leads to use:
- Hugging Face lookup for Qwen3 Next 80B-A3B: https://huggingface.co/models?search=Qwen3+Next+80B-A3B
- Local inventory provenance: /Users/rigarg/Desktop/agents/model-atlas/oss-model-releases-2025-plus.md
...
- "Qwen3 Next 80B-A3B" "model card"
- "Qwen3 Next 80B-A3B" "technical report"
- "Qwen3 Next 80B-A3B" "architecture" "config"
- "Qwen3 Next 80B-A3B" "benchmark" "Alibaba Qwen"

Primary-source checklist for this model:
- Official model card or repository from Alibaba Qwen.
- Official technical report, paper, release blog, or documentation page for Qwen3 Next 80B-A3B.
...
- Release/report date: 2025-09-09
- Release year: 2025
- Parameter scale: 80B total
- Active parameter scale: 3B active
- License or openness: Apache-2.0
- Inventory source strength: Raschka table
- Target page title: "Qwen3 Next 80B-A3B Architecture"
...
Goal:
Build a visual architecture atlas page that explains Qwen3 Next 80B-A3B from input tokens or modality inputs to output behavior. The page should be accurate to verified sources, visually rich, and specific to Qwen3 Next 80B-A3B. Do not make a generic transformer explainer.
...
- The first viewport should immediately identify Qwen3 Next 80B-A3B and show the high-level specs from the inventory.
...
- Mirror the verified report or model-card structure for Qwen3 Next 80B-A3B when available.
- Avoid generic statements like "uses transformer blocks" unless paired with the concrete mechanism and dimensions verified for Qwen3 Next 80B-A3B.
```
