# Claude Design Prompt - SmolLM3 3B
...
- Architecture / modality note: Dense Transformer; GQA; periodic NoPE; decoder-only.
- Source strength: Raschka table
- Source lead: https://huggingface.co/models?search=SmolLM3+3B
...
```text
You are Claude Design. Create a standalone, downloadable HTML visualization page for SmolLM3 3B.

Primary source leads to use:
- Hugging Face lookup for SmolLM3 3B: https://huggingface.co/models?search=SmolLM3+3B
- Local inventory provenance: /Users/rigarg/Desktop/agents/model-atlas/oss-model-releases-2025-plus.md
...
- "SmolLM3 3B" "technical report"
- "SmolLM3 3B" "architecture" "config"
- "SmolLM3 3B" "benchmark" "Hugging Face"

Primary-source checklist for this model:
- Official model card or repository from Hugging Face.
- Official technical report, paper, release blog, or documentation page for SmolLM3 3B.
...
Goal:
Build a visual architecture atlas page that explains SmolLM3 3B from input tokens or modality inputs to output behavior. The page should be accurate to verified sources, visually rich, and specific to SmolLM3 3B. Do not make a generic transformer explainer.
...
2. Block internals:
   - Draw the block structure implied by verified sources and the inventory note: Dense Transformer; GQA; periodic NoPE; decoder-only.
...
3. Attention and sequence handling:
   ...
   - Explain positional encoding. Inventory lead: NoPE.
...
- The first viewport should immediately identify SmolLM3 3B and show the high-level specs from the inventory.
...
- Mirror the verified report or model-card structure for SmolLM3 3B when available.
- Avoid generic statements like "uses transformer blocks" unless paired with the concrete mechanism and dimensions verified for SmolLM3 3B.
```
