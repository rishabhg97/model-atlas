...
- Lab: NXAI
- Size / active size: 7B
- License / openness: NXAI Community License
- Architecture / modality note: Recurrent xLSTM language model; mLSTM blocks.
- Source strength: Raschka table
- Source lead: https://huggingface.co/models?search=xLSTM+7B
...
```text
You are Claude Design. Create a standalone, downloadable HTML visualization page for xLSTM 7B.

Primary source leads to use:
- Hugging Face lookup for xLSTM 7B: https://huggingface.co/models?search=xLSTM+7B
- Local inventory provenance: /Users/rigarg/Desktop/agents/model-atlas/oss-model-releases-2025-plus.md
...
- "xLSTM 7B" "model card"
- "xLSTM 7B" "technical report"
- "xLSTM 7B" "architecture" "config"
- "xLSTM 7B" "benchmark" "NXAI"

Primary-source checklist for this model:
- Official model card or repository from NXAI.
- Official technical report, paper, release blog, or documentation page for xLSTM 7B.
...
Goal:
Build a visual architecture atlas page that explains xLSTM 7B from input tokens or modality inputs to output behavior. The page should be accurate to verified sources, visually rich, and specific to xLSTM 7B. Do not make a generic transformer explainer.
...
2. Block internals:
   - Draw the block structure implied by verified sources and the inventory note: Recurrent xLSTM language model; mLSTM blocks.
...
5. Model-specific sections to create from the inventory leads:
   - Recurrent xLSTM language model
   - mLSTM blocks
...
- The first viewport should immediately identify xLSTM 7B and show the high-level specs from the inventory.
...
- Mirror the verified report or model-card structure for xLSTM 7B when available.
- Avoid generic statements like "uses transformer blocks" unless paired with the concrete mechanism and dimensions verified for xLSTM 7B.
```
