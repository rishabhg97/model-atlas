...
- Architecture / modality note: Sparse MoE reasoning model; CCA plus GQA attention.
- Source strength: Raschka table
- Source lead: https://huggingface.co/models?search=ZAYA1-8B
...
```text
You are Claude Design. Create a standalone, downloadable HTML visualization page for ZAYA1-8B.

Primary source leads to use:
- Hugging Face lookup for ZAYA1-8B: https://huggingface.co/models?search=ZAYA1-8B
- Local inventory provenance: /Users/rigarg/Desktop/agents/model-atlas/oss-model-releases-2025-plus.md
...
- "ZAYA1-8B" "architecture" "config"
- "ZAYA1-8B" "benchmark" "Zyphra"

Primary-source checklist for this model:
- Official model card or repository from Zyphra.
- Official technical report, paper, release blog, or documentation page for ZAYA1-8B.
...
Goal:
Build a visual architecture atlas page that explains ZAYA1-8B from input tokens or modality inputs to output behavior. The page should be accurate to verified sources, visually rich, and specific to ZAYA1-8B. Do not make a generic transformer explainer.
...
2. Block internals:
   - Draw the block structure implied by verified sources and the inventory note: Sparse MoE reasoning model; CCA plus GQA attention.
...
5. Model-specific sections to create from the inventory leads:
   - Sparse MoE reasoning model
   - CCA plus GQA attention
...
- The first viewport should immediately identify ZAYA1-8B and show the high-level specs from the inventory.
...
- Mirror the verified report or model-card structure for ZAYA1-8B when available.
- Avoid generic statements like "uses transformer blocks" unless paired with the concrete mechanism and dimensions verified for ZAYA1-8B.
```
